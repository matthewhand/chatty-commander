...
+ eval find . -maxdepth 2 '\(' -path '"./.git"' -o -path '"./node_modules"' -o -path '"./dist"' -o -path '"./build"' -o -path '"./__pycache__"' -o -path '"./logs"' -o -path '"./.venv"' -o -path '"./.venv_*"' -o -path '"./.mypy_cache"' '\)' -prune -o -type d -print
++ find . -maxdepth 2 '(' -path ./.git -o -path ./node_modules -o -path ./dist -o -path ./build -o -path ./__pycache__ -o -path ./logs -o -path ./.venv -o -path './.venv_*' -o -path ./.mypy_cache ')' -prune -o -type d -print
+ sed 's|^\./||'
+ sort
.
app
...
+ sed -n 1,300p
    ...
    93	  ```powershell
    94	  .\scripts\windows\start-web.ps1 -Port 8100 -NoAuth
    95	  ```
    96	- macOS (Terminal):
    97	  ```bash
    98	  chmod +x scripts/macos/start-web.sh
    99	  PORT=8100 NO_AUTH=1 scripts/macos/start-web.sh
   100	  ```
   101
   ...
   105
   106	```bash
   107	uv run python main.py --web --port 8100
   108	```
   109
   ...
   120
   121	```bash
   122	uv run python main.py --web --no-auth --port 8100
   123	```
   124
   125	This enables:
   126
   ...
   179	  ```bash
   180	  uv run python main.py --orchestrate --enable-text --web --no-auth --port 8100
   181	  ```
   182	- Web + Discord bridge (advisors enabled):
   183	  ```bash
   184	  export ADVISORS_BRIDGE_TOKEN=secret
   185	  export ADVISORS_BRIDGE_URL=http://localhost:3001
   186	  # ensure advisors.enabled=true in config
   187	  uv run python main.py --orchestrate --web --enable-discord-bridge --advisors --port 8100
   188	  ```
   189	- Text only (headless dev):
   190	  ```bash
   191	  uv run python main.py --orchestrate --enable-text
   192	  ```
   193
   194	## Quickstart
   195
   ...
   199	```
   200	uv run python -m src.chatty_commander.cli.cli list
   201	```
   202
   203	- Execute a command in dry-run mode (no side effects)
   204	```
   205	uv run python -m src.chatty_commander.cli.cli exec hello --dry-run
   206	```
   207
   208	- Start interactive shell
   209	```
   210	uv run python -m src.chatty_commander.cli.cli
   211	```
   212
   ...
   216	```
   217	uv run python -m src.chatty_commander.main --web --no-auth --port 8100
   218	```
   219
   ...
   223	```
   224	curl -s http://localhost:8100/api/v1/health | jq
   225	curl -s http://localhost:8100/api/v1/status | jq
   226	curl -s http://localhost:8100/api/v1/version | jq
   227	```
   228
   229	- Get/Put config and change state:
   230	```
   231	curl -s http://localhost:8100/api/v1/config | jq
   232	curl -s -X PUT http://localhost:8100/api/v1/config -H 'Content-Type: application/json' -d '{"foo":{"bar":1}}'
   233	curl -s -X POST http://localhost:8100/api/v1/state -H 'Content-Type: application/json' -d '{"state":"computer"}' | jq
   234	```
   235
   236	- Metrics:
   237	```
   238	curl -s http://localhost:8100/metrics/json | jq
   239	curl -s http://localhost:8100/metrics/prom | head -n 20
   240	```
   241
   ...
   245	```
   246	uv sync --group voice
   247	```
   248
   249	- Test voice pipeline:
   250	```
   251	uv run python -m src.chatty_commander.cli.cli voice test --mock --duration 10
   252	```
   253
   254	- Check voice system status:
   255	```
   256	uv run python -m src.chatty_commander.cli.cli voice status
   257	```
   258
   259	- Demo voice integration:
   260	```
   261	python scripts/voice_demo.py --mock
   262	```
   263
   264	### WebUI
   265
   ...
   273	```
   274	uv run python -m src.chatty_commander.cli.cli list
   275	uv run python -m src.chatty_commander.cli.cli exec hello --dry-run
   276	```
   277
   278	2) Update config and switch states via Web API
   279	```
   280	curl -s -X PUT http://localhost:8100/api/v1/config -H 'Content-Type: application/json' -d '{"foo":{"bar":1}}'
   281	curl -s -X POST http://localhost:8100/api/v1/state -H 'Content-Type: application/json' -d '{"state":"computer"}' | jq
   282	```
   283
   284	3) Observe metrics
   285	```
   286	curl -s http://localhost:8100/metrics/json | jq
   287	```
   288
   289	4) Agents: create, list, update, delete
   290	```
   291	# Create
   292	curl -s -X POST http://localhost:8100/api/v1/agents/blueprints -H 'Content-Type: application/json' -d '{"description":"Summarizer agent"}' | jq
   293	# List
   294	curl -s http://localhost:8100/api/v1/agents/blueprints | jq
   295	# Update (replace <ID>)
   296	curl -s -X PUT http://localhost:8100/api/v1/agents/blueprints/<ID> -H 'Content-Type: application/json' -d '{"name":"SummarizerX","description":"d","persona_prompt":"p","capabilities":[],"team_role":null,"handoff_triggers":[]}' | jq
   297	# Delete
   298	curl -s -X DELETE http://localhost:8100/api/v1/agents/blueprints/<ID> | jq
   299	```
   300
...
+ sed -n 1,200p
    ...
    19	- [ ] **Native Installers** – experiment with PyInstaller, cx_Freeze, Electron-builder, pkg, and similar tools to generate platform-specific binaries.
    20
    ...
    27	- [ ] **Log Tail & Summarizer** – buffer last _N_ lines, invoke LLM to output enforced 3-word summaries at intervals.
...
+ sed -n 1,200p
     ...
     7
     8	```bash
     9	uv sync --all-groups
    10	uv run pydocstyle src
    11	uv run doc8 README.md docs
    12	uv run pytest --check-links README.md docs -q
    13	```
    14
...
+ sed -n 1,300p
    ...
    30
    31	[project.scripts]
    32	chatty-commander = "chatty_commander.cli.cli:main"
    33
   ...
   110	  "D107", # Missing docstring in __init__
...
++ tr -d ' '
+ py_count=80
++ find src webui -type f '(' -name '*.ts' -o -name '*.tsx' -o -name '*.js' ')'
++ wc -l
++ tr -d ' '
+ ts_count=70307
...
+ sed -n 1,120p
    ...
    18	python tests/test_system.py --verbose --output-file "test_results_$(date +%Y%m%d_%H%M%S).txt"
    19
    20	echo "================================================"
...
+ cap=250
+ shift
+ echo '$ grep -RInE '\''TODO|FIXME|HACK|XXX'\'' -- . | grep -vE '\''node_modules|dist|build|__pycache__|.git'\'' || true'
$ grep -RInE 'TODO|FIXME|HACK|XXX' -- . | grep -vE 'node_modules|dist|build|__pycache__|.git' || true
++ eval 'grep -RInE '\''TODO|FIXME|HACK|XXX'\'' -- . | grep -vE '\''node_modules|dist|build|__pycache__|.git'\'' || true'
++ sed -n 1,250p
+++ grep -RInE 'TODO|FIXME|HACK|XXX' -- .
+++ grep -vE 'node_modules|dist|build|__pycache__|.git'
+ out='./reports/triage_2025-08-16T08-32-33+10-00.md:602:+ showfile TODO.md 200
...
./.venv_prmerge/lib/python3.11/site-packages/mpmath/__init__.py:22:# XXX: extremely bad pickle hack
...
./.venv_prmerge/lib/python3.11/site-packages/mpmath/functions/bessel.py:878:                # TODO: use v <= j'\''_{v,1} < y_{v,1}?
...
./.venv_prmerge/lib/python3.11/site-packages/mpmath/calculus/__init__.py:2:# XXX: hack to set methods
...
./.venv_prmerge/lib/python3.11/site-packages/mpmath/libmp/gammazeta.py:626:TODO: the current estimation of N for m > 0 is *very suboptimal*.
...
./.venv_prmerge/lib/python3.11/site-packages/iniconfig/__init__.py:80:    # TODO: investigate possible mypy bug wrt matching the passed over data
...
./.venv_prmerge/lib/python3.11/site-packages/pip/_vendor/pygments/filters/__init__.py:72:       highlight ``XXX``, ``TODO``, ``FIXME``, ``BUG`` and ``NOTE``.
./.venv_prmerge/lib/python3.11/site-packages/pip/_vendor/pygments/filters/__init__.py:75:       Now recognizes ``FIXME`` by default.
./.venv_prmerge/lib/python3.11/site-packages/pip/_vendor/pygments/filters/__init__.py:81:                            ['\''XXX'\'', '\''TODO'\'', '\''FIXME'\'', '\''BUG'\'', '\''NOTE'\''])
...
./.venv_prmerge/lib/python3.11/site-packages/pip/_vendor/pkg_resources/__init__.py:1:# TODO: Add Generic type annotations to initialized collections.
./.venv_prmerge/lib/python3.11/site-packages/pip/_vendor/pkg_resources/__init__.py:122:_ResourceStream = Any  # TODO / Incomplete: A readable file-like object
./.venv_prmerge/lib/python3.11/site-packages/pip/_vendor/pkg_resources/__init__.py:478:# XXX backward compat
./.venv_prmerge/lib/python3.11/site-packages/pip/_vendor/pkg_resources/__init__.py:487:    XXX Needs compatibility checks for Linux and other unixy OSes.
./.venv_prmerge/lib/python3.11/site-packages/pip/_vendor/pkg_resources/__init__.py:527:    # XXX Linux and other platforms'\'' special cases should go here
./.venv_prmerge/lib/python3.11/site-packages/pip/_vendor/pkg_resources/__init__.py:723:            # XXX add more info
./.venv_prmerge/lib/python3.11/site-packages/pip/_vendor/pkg_resources/__init__.py:1319:# XXX backward compatibility
./.venv_prmerge/lib/python3.11/site-packages/pip/_vendor/pkg_resources/__init__.py:1522:        # XXX
./.venv_prmerge/lib/python3.11/site-packages/pip/_vendor/pkg_resources/__init__.py:2031:    # FIXME: '\''ZipProvider._extract_resource'\'' is too complex (12)
./.venv_prmerge/lib/python3.11/site-packages/pip/_vendor/pkg_resources/__init__.py:3201:    # FIXME: '\''Distribution.insert_on'\'' is too complex (13)
./.venv_prmerge/lib/python3.11/site-packages/pip/_vendor/pkg_resources/__init__.py:3308:            # TODO: remove this except clause when python/cpython#103632 is fixed.
./.venv_prmerge/lib/python3.11/site-packages/pip/_vendor/pkg_resources/__init__.py:3598:        # TODO: Add a deadline?
...
+ cap=200
+ shift
+ echo '$ grep -RInE '\''TypeError|ImportError|ModuleNotFoundError|Unhandled|UnhandledPromiseRejection|DeprecationWarning'\'' -- logs tests test_results_*.txt 2>/dev/null || true'
$ grep -RInE 'TypeError|ImportError|ModuleNotFoundError|Unhandled|UnhandledPromiseRejection|DeprecationWarning' -- logs tests test_results_*.txt 2>/dev/null || true
++ eval 'grep -RInE '\''TypeError|ImportError|ModuleNotFoundError|Unhandled|UnhandledPromiseRejection|DeprecationWarning'\'' -- logs tests test_results_*.txt 2>/dev/null || true'
...
++ sed -n 1,200p
+ out='logs/pytest_baseline_pip.txt:1185:    import warnings as _w; _w.warn("config.py is deprecated; use chatty_commander.app.config", DeprecationWarning); from chatty_commander.app.config import *  # noqa
logs/pytest_baseline_pip.txt:1776:    import warnings as _w; _w.warn("config.py is deprecated; use chatty_commander.app.config", DeprecationWarning); from chatty_commander.app.config import *  # noqa
