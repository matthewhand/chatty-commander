...
## 2024-05-15 - [Nested Dictionary Generator Expression Optimization]
**Learning:** In highly-recursive utility functions like `mask_sensitive_data` which process nested dictionaries, placing generator expressions like `any(...)` inside an inner loop forces redundant re-computations and generator allocation overhead.
**Action:** Extract static collections into module-level tuples, cache expensive string operations before the loop, and use explicit `for` loops with early `break` statements to improve performance.
