...
## 2024-05-18 - Path Traversal Authentication Bypass
**Vulnerability:** A path traversal vulnerability existed in the authentication middleware due to the behavior of `posixpath.normpath()`. `normpath()` collapses paths with three or more leading slashes (e.g., `///api`) but preserves paths with exactly two leading slashes (e.g., `//api`) for POSIX compliance. Since the auth check used `path.startswith('/api/')`, an attacker could bypass authentication by requesting `//api/...` which `normpath()` would leave as `//api/...`, bypassing the `/api/` prefix check, but the underlying framework would still route to the protected endpoint.
**Learning:** `posixpath.normpath()` is not sufficient on its own for security-critical path normalization when prefix checking is used, because of its specific preservation of double leading slashes. Frameworks often normalize these paths away during routing, creating a discrepancy between middleware evaluation and actual routing.
**Prevention:** When normalizing paths for security checks (especially prefix matching), explicitly strip or replace any remaining leading double slashes after calling `posixpath.normpath()`. Ensure the path strictly begins with a single slash (or matches expected routing logic exactly).
## 2024-05-15 - [Deep Dictionary Key Sanitization]
**Vulnerability:** The configuration sanitization logic checked nested dictionary values but failed to validate dictionary keys against dangerous strings.
**Learning:** Injection attacks can be hidden within dictionary keys (e.g. `__proto__` injection in configuration merges) just as easily as within values. Recursive validation must apply equally to both keys and values.
**Prevention:** Implement recursive key and value checking for all untrusted dictionary structures being ingested or validated.
