...
_ENV_VAR_CATALOG: dict[str, tuple[str, str | None, bool]] = {
    ...
    "OPENAI_API_BASE": (
        "Legacy base URL for OpenAI-compatible API (fallback for OPENAI_BASE_URL)",
        "https://api.openai.com/v1",
        False,
    ),
    ...
    # Agents
    "CHATTY_AGENTS_STORE": (
        "Path to the agents store JSON file",
        "~/.chatty_commander/agents.json",
        False,
    ),
    # Bridge / API endpoints
    "CHATTY_BRIDGE_TOKEN": (
        "Authentication token for the web-server bridge",
        None,
        False,
    ),
    "CHATBOT_ENDPOINT": ("Override URL for the chatbot API endpoint", None, False),
    "HOME_ASSISTANT_ENDPOINT": (
        "Override URL for the Home Assistant API endpoint",
        None,
        False,
    ),
    # General settings
    ...
    "CHATCOMM_INFERENCE_FRAMEWORK": (
        "Inference framework to use (e.g. onnx)",
        "onnx",
        False,
    ),
    "CHATCOMM_START_ON_BOOT": (
        "Start application on system boot (true/yes/1)",
        None,
        False,
    ),
    "CHATCOMM_CHECK_FOR_UPDATES": (
        "Enable automatic update checks (true/yes/1)",
        None,
        False,
    ),
    # CLI
