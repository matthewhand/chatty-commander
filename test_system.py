...
class SystemTester:
    def __init__(self, verbose=False, output_file=None):
        ...
        print(f"\n{'='*60}")
        print(f"ChattyCommander System Testing Suite")
    ...
    def test_system_management(self):
        ...
        if result['success']:
            ...
            self.log(f"✗ Start-on-boot status failed: {result['stderr']}", "System Management", "FAIL")
        ...
        if result['success']:
            ...
            self.log(f"✗ Start-on-boot enable failed: {result['stderr']}", "System Management", "FAIL")
        ...
        if result['success']:
            ...
            self.log(f"✗ Start-on-boot disable failed: {result['stderr']}", "System Management", "FAIL")
        ...
        if result['success']:
            ...
            self.log(f"✗ Auto-update enable failed: {result['stderr']}", "System Management", "FAIL")
        ...
        if result['success']:
            ...
            self.log(f"✗ Auto-update disable failed: {result['stderr']}", "System Management", "FAIL")
    ...
    def test_state_transitions(self):
        ...
        try:
            ...
            if state_manager.current_state == 'idle':
                ...
                self.log(f"✗ Initial state is '{state_manager.current_state}', expected 'idle'", "State Transitions", "FAIL")
            ...
            for command, expected_state in transitions:
                ...
                if new_state == expected_state:
                    self.log(f"✓ Command '{command}' transitions to '{expected_state}'", "State Transitions", "PASS")
                else:
                    self.log(f"✗ Command '{command}' failed to transition to '{expected_state}', got '{new_state}'", "State Transitions", "FAIL")
            
            # Test invalid state
            try:
                state_manager.change_state('invalid_state')
                self.log("✗ Invalid state change should raise ValueError", "State Transitions", "FAIL")
            except ValueError:
                self.log("✓ Invalid state change raises ValueError correctly", "State Transitions", "PASS")
    ...
    def test_config_loading(self):
        ...
        try:
            ...
            required_attrs = [
                'model_actions', 'state_models', 'api_endpoints',
                'general_models_path', 'system_models_path', 'chat_models_path',
                'debug_mode', 'default_state'
            ]
            ...
            for attr in required_attrs:
                if hasattr(config, attr):
                    ...
                    self.log(f"✗ Config missing required attribute '{attr}'", "Config Loading", "FAIL")
            
            # Test state models configuration
            expected_states = ['idle', 'computer', 'chatty']
            for state in expected_states:
                if state in config.state_models:
                    self.log(f"✓ State '{state}' configured with models: {config.state_models[state]}", "Config Loading", "PASS")
                else:
                    self.log(f"✗ State '{state}' not configured", "Config Loading", "FAIL")
            
            # Test model actions
            expected_commands = [
                'take_screenshot', 'paste', 'submit', 'cycle_window',
                'oh_kay_screenshot', 'wax_poetic', 'lights_on', 'lights_off'
            ]
            
            for command in expected_commands:
                if command in config.model_actions:
                    action = config.model_actions[command]
                    self.log(f"✓ Command '{command}' configured: {action}", "Config Loading", "PASS")
                else:
        ...
        try:
            ...
            for state in ['idle', 'computer', 'chatty']:
                try:
                    ...
                    self.log(f"✗ Failed to load models for state '{state}': {str(e)}", "Model Manager", "FAIL")
        ...
        try:
            ...
            if valid_commands:
                ...
                if executor.validate_command(test_command):
                    self.log(f"✓ Command validation works for '{test_command}'", "Command Executor", "PASS")
                else:
                    self.log(f"✗ Command validation failed for '{test_command}'", "Command Executor", "FAIL")
            ...
            try:
                ...
                self.log("✓ Invalid command correctly rejected with exception", "Command Executor", "PASS")
    ...
    def test_gui_launch(self):
        ...
        result = self.run_command('timeout 5 chatty gui', expected_exit_code=124)  # timeout exit code
        if result['returncode'] == 124:  # Command was terminated by timeout
            self.log("✓ GUI launches successfully (terminated by timeout as expected)", "GUI Launch", "PASS")
        elif result['success']:
    ...
    def test_installation(self):
        ...
        if result['success'] and result['stdout'].strip():
            self.log(f"✓ 'chatty' command available at: {result['stdout'].strip()}", "Installation", "PASS")
        else:
        ...
        for module in modules:
            ...
            if result['success'] and 'OK' in result['stdout']:
                ...
                self.log(f"✗ Module '{module}' import failed: {result['stderr']}", "Installation", "FAIL")
    ...
    def run_all_tests(self):
        ...
        for test_method in test_methods:
            try:
                ...
                self.log(f"✗ Test method {test_method.__name__} crashed: {str(e)}", test_method.__name__, "ERROR")
...
def main():
    ...
    parser.add_argument('--mode', choices=['all', 'cli', 'config', 'states', 'system', 'gui'], 
