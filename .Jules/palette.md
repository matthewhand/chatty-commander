...
## 2024-05-04 - Consistent Clear Icons
**Learning:** Raw text characters like 'x' or '×' are often used for close/clear buttons, but they lack visual consistency with other interface icons and can sometimes cause slight vertical alignment issues.
**Action:** When implementing close or clear buttons, replace raw text characters with proper vector icons (e.g., `X` from `lucide-react`) to maintain a cohesive design system and improve vertical alignment within buttons. Always ensure they have a visible `title` attribute for tooltip functionality alongside `aria-label`.
