...
## 2026-06-02 - Tooltip Hover Targets on Disabled Elements
**Learning:** In DaisyUI/Tailwind applications, disabled `<button>` elements swallow mouse hover events, which prevents attached tooltips from triggering. Wrapping the button in a standard component isn't enough; the tooltip attributes need to be on an enabled parent container.
**Action:** When adding a tooltip to a button that conditionally disables, wrap the button in a `div` carrying the `.tooltip` class and `data-tip` attribute, and explicitly set `style={{ pointerEvents: 'none' }}` on the disabled button so hover events bubble up to the wrapper.
