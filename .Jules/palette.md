...
## 2026-04-05 - Custom UI Wrapper Components and aria-label
**Learning:** Custom UI wrapper components (like `Dropdown` from our DaisyUI wrapper) that trigger actions via an icon-only button often lack standard accessibility props like `aria-label`. This makes the action opaque to screen readers.
**Action:** When working with custom UI wrapper components that have icon-only triggers, ensure they accept an optional `ariaLabel` prop and pass it down to the native HTML element (e.g. the trigger `div` or `button`). Add this prop where the component is used (e.g., `ariaLabel="Actions for [Command Name]"` in lists).
