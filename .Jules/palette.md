...
## 2024-04-14 - [Disabled Button Tooltips]
**Learning:** In DaisyUI/Tailwind, disabled buttons swallow hover events, which prevents informative tooltips from appearing to explain *why* the button is disabled.
**Action:** Wrap disabled buttons in a div containing the tooltip classes and dynamically apply `style={{ pointerEvents: 'none' }}` to the button itself when it is disabled, allowing the wrapper div to intercept the hover event and show the helpful tip.
