...
- [x] **No random test ordering** — order-dependence is hidden (module-global stores + the leaking stubs); add `pytest-randomly` to surface coupling ✅ (#735 — added `pytest-randomly`; it immediately surfaced 3 isinstance-across-reload failures from `test_switch_mode_tool.py`/`test_dograh_advisor_tool.py` reloading the shared `advisors.providers` module in place. Fixed both fixtures to snapshot each module's `__dict__` and restore the original class objects on teardown; suite now green across seeds 1/12345/99999) ([`pyproject.toml`](pyproject.toml), [`tests/`](tests/))
