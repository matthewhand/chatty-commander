...
class TestPerformanceBenchmarks:
    ...
    def test_state_transition_performance(self, state_manager):
        ...
        assert rapid_transition_time < 0.1, f"100 rapid transitions took {rapid_transition_time:.3f}s"
    ...
    def test_model_loading_performance(self, mock_listdir, mock_exists, model_manager):
        ...
        with patch('model_manager.Model', return_value=MagicMock()) as mock_model:
            ...
            assert execution_time < 0.5, f"Model loading took {execution_time:.3f}s, expected < 0.5s"
    ...
    def test_command_execution_performance(self, command_executor):
        ...
        for command_name in test_command_names:
            with patch.object(command_executor, '_execute_keybinding'), \
    ...
    def test_memory_leak_detection(self, config):
        ...
        assert memory_growth < 5 * 1024 * 1024, f"Memory grew by {memory_growth} bytes after 100 operations"
    ...
    def test_startup_performance(self):
        ...
        with patch('model_manager.os.path.exists', return_value=True), \
