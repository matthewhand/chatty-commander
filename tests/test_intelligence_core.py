...
class TestAIResponse:
    ...
    def test_ai_response_as_dict(self):
        ...
        timestamp = datetime.now()
        response = AIResponse(
            text="Test", confidence=0.8, intent="test", actions=[], metadata={}, timestamp=timestamp
        )
...
class TestIntelligenceCore:
    ...
    def test_handle_voice_input_low_confidence(
        ...
        with patch('src.chatty_commander.ai.intelligence_core.create_enhanced_voice_processor'):
            core = IntelligenceCore(mock_config)
    ...
    def test_handle_voice_input_high_confidence(
        ...
        with patch('src.chatty_commander.ai.intelligence_core.create_enhanced_voice_processor'):
            ...
            with patch.object(core, 'process_input', return_value=mock_response) as mock_process:
                core._handle_voice_input(voice_result)
    ...
    @patch('src.chatty_commander.ai.intelligence_core.AdvisorsService')
    @patch('src.chatty_commander.ai.intelligence_core.StateManager')
    def test_handle_voice_input_with_callback(self, mock_state_manager, mock_advisors, mock_config):
    ...
    @patch('src.chatty_commander.ai.intelligence_core.AdvisorsService')
    @patch('src.chatty_commander.ai.intelligence_core.StateManager')
    def test_handle_voice_input_with_error(self, mock_state_manager, mock_advisors, mock_config):
        ...
        with patch('src.chatty_commander.ai.intelligence_core.create_enhanced_voice_processor'):
            ...
            with patch.object(core, 'process_input', side_effect=Exception("Processing failed")):
                core._handle_voice_input(voice_result)

                error_callback.assert_called_once_with("Voice processing error: Processing failed")

    @patch('src.chatty_commander.ai.intelligence_core.AdvisorsService')
    @patch('src.chatty_commander.ai.intelligence_core.StateManager')
    def test_handle_wake_word_chatty(self, mock_state_manager, mock_advisors, mock_config):
    ...
    @patch('src.chatty_commander.ai.intelligence_core.AdvisorsService')
    @patch('src.chatty_commander.ai.intelligence_core.StateManager')
    def test_handle_wake_word_computer(self, mock_state_manager, mock_advisors, mock_config):
    ...
    @patch('src.chatty_commander.ai.intelligence_core.AdvisorsService')
    @patch('src.chatty_commander.ai.intelligence_core.StateManager')
    def test_handle_wake_word_same_mode(self, mock_state_manager, mock_advisors, mock_config):
    ...
    def test_handle_wake_word_state_change_error(
        ...
        with patch('src.chatty_commander.ai.intelligence_core.create_enhanced_voice_processor'):
            ...
            mock_state_manager_instance = mock_state_manager.return_value
            mock_state_manager_instance.current_state = "computer"
            mock_state_manager_instance.change_state.side_effect = Exception("State change failed")
            core.state_manager = mock_state_manager_instance
    ...
    def test_process_input_text(self, mock_state_manager, mock_advisors, mock_config):
        ...
        with patch('src.chatty_commander.ai.intelligence_core.create_enhanced_voice_processor'):
            ...
            with patch.object(core, '_extract_actions', return_value=[]) as mock_extract:
                with patch.object(core, '_analyze_intent', return_value="greeting") as mock_analyze:
    ...
    def test_process_input_voice_with_metadata(
        ...
        with patch('src.chatty_commander.ai.intelligence_core.create_enhanced_voice_processor'):
            ...
            with patch.object(core, '_extract_actions', return_value=[{"type": "speak"}]):
    ...
    @patch('src.chatty_commander.ai.intelligence_core.AdvisorsService')
    @patch('src.chatty_commander.ai.intelligence_core.StateManager')
    def test_process_input_error_handling(self, mock_state_manager, mock_advisors, mock_config):
        ...
        with patch('src.chatty_commander.ai.intelligence_core.create_enhanced_voice_processor'):
            ...
            mock_advisors_instance = mock_advisors.return_value
            mock_advisors_instance.handle_message.side_effect = Exception("Service error")
            core.advisors_service = mock_advisors_instance
