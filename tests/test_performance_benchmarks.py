...
class TestPerformanceBenchmarks:
    ...
    def test_api_response_time_benchmark(self, test_client):
        ...
        for endpoint, stats in results.items():
            ...
            assert stats['mean'] < 100, f"{endpoint} mean response time too high: {stats['mean']:.2f}ms"
            assert stats['median'] < 50, f"{endpoint} median response time too high: {stats['median']:.2f}ms"
    ...
    def test_websocket_message_broadcast_performance(self, web_server):
        ...
        try:
            ...
            assert broadcast_time/num_connections < 2, f"Per-connection time too high: {broadcast_time/num_connections:.2f}ms"
        finally:
            loop.close()
