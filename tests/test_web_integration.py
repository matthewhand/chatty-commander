                ...
                for field in required_fields:
                    if field not in data:
                        self.test_results["errors"].append(
                            f"Health endpoint missing field: {field}"
                        )
                        return False

                if data["status"] != "healthy":
                    self.test_results["errors"].append(
                        f"Health status not healthy: {data['status']}"
                    )
                    return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for field in required_fields:
                    if field not in data:
                        self.test_results["errors"].append(
                            f"Status endpoint missing field: {field}"
                        )
                        return False
    ...
    def test_command_endpoint(self) -> bool:
        ...
        try:
            ...
            if response.status_code != 404:
                self.test_results["errors"].append(
                    f"Command endpoint should return 404 for invalid command, got {response.status_code}"
                )
                return False
            ...
            if response.status_code == 404:
                ...
                self.test_results["errors"].append(
                    f"Command endpoint unexpected response: {response.status_code}"
                )
                return False
            ...
            async with websockets.connect(self.ws_url) as websocket:
                ...
                try:
                    ...
                    if data.get("type") != "connection_established":
                        self.test_results["errors"].append(
                            f"Expected connection_established, got {data.get('type')}"
                        )
                        return False
                    ...
                    if response_data.get("type") != "pong":
                        self.test_results["errors"].append(
                            f"Expected pong response, got {response_data.get('type')}"
                        )
                        return False
