...
interface CardTitleProps { children?: ReactNode; className?: string; tag?: 'h2' | 'h3' | 'div' | 'span' | 'h4' | 'h5' | 'h6'; }
interface CardActionsProps { children?: ReactNode; className?: string; }
...
const CardBase: React.FC<CardProps> = ({
  title, subtitle, children, actions, imageSrc, imageAlt = 'Card image',
  imageOverlay = false, compact = false, side = false, imageFull = false,
  bgVariant, borderVariant, loading = false, emptyState, hover = false,
  glowColor, className = '', ...props
}) => {
  ...
  if (hover) {
    ...
    if (glowColor && glowMap[glowColor]) cardClasses += ` ${glowMap[glowColor]}`;
  }
  ...
  if (loading) {
    return (
      <div className={cardClasses} role="article" {...props}>
        {imageSrc && !imageOverlay && <figure className="skeleton h-32 w-full" />}
        <div className="card-body">
          <div className="skeleton h-4 w-1/3 mb-2" />
          <div className="skeleton h-4 w-full mb-2" />
          <div className="skeleton h-4 w-full mb-4" />
          <div className="skeleton h-8 w-1/4 ml-auto" />
        </div>
      </div>
    );
  }
  ...
  if (emptyState) {
    return (
      <div className={cardClasses} {...props}>
        {imageSrc && !imageOverlay && <figure><img src={imageSrc} alt={imageAlt} /></figure>}
  ...
  return (
    <div className={cardClasses} {...props}>
      {imageSrc && imageFull && <figure><img src={imageSrc} alt={imageAlt} /></figure>}
      {imageSrc && imageOverlay && (
        <figure className="absolute inset-0 z-0"><img src={imageSrc} alt={imageAlt} className="w-full h-full object-cover" /></figure>
      )}
      {imageSrc && !imageFull && !imageOverlay && <figure><img src={imageSrc} alt={imageAlt} /></figure>}
