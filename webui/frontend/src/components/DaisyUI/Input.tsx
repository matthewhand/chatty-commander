...
export const Input = memo(forwardRef<HTMLInputElement, InputProps>(
  ({ variant, size = 'md', bordered = true, ghost = false, loading = false,
    disabled, prefix, suffix, className, containerClassName, label, error,
    helperText, id: providedId, type = 'text', ...props }, ref) => {
    ...
    const variantClasses: Record<string, string> = {
      primary: 'input-primary', secondary: 'input-secondary', accent: 'input-accent',
      info: 'input-info', success: 'input-success', warning: 'input-warning', error: 'input-error',
    };
    const sizeClasses: Record<string, string> = {
      xs: 'input-xs', sm: 'input-sm', md: 'input-md', lg: 'input-lg',
    };
    ...
    const inputContent = (
      <div className={twMerge('relative', containerClassName)}>
        ...
        <input
          ...
          disabled={disabled || loading} className={inputClasses}
          aria-invalid={!!error}
        ...
        {(isPassword || suffix || loading) && (
          <div className="absolute inset-y-0 right-0 flex items-center pr-3 gap-2 pointer-events-none z-10">
            ...
            {suffix && <div className="flex items-center pointer-events-auto"><span className="text-base-content/60 sm:text-sm">{suffix}</span></div>}
            {loading && <div className="flex items-center"><span className="loading loading-spinner w-4 h-4" aria-hidden="true" /></div>}
          </div>
        )}
      </div>
    );
