...
class TestRunner:
    ...
    def run_command(self, command: List[str], description: str, cwd: Optional[Path] = None) -> Tuple[bool, str]:
    ...
    def check_dependencies(self) -> bool:
        ...
        success, _ = self.run_command(["uv", "run", "python", "-c", "import pytest_cov"], "pytest-cov availability")
        if not success:
    ...
    def run_linting(self) -> bool:
        ...
        try:
            ...
            if success:
                # Run flake8 linting
                command = ["uv", "run", "flake8", ".", "--max-line-length=120", "--exclude=archive,webui/frontend"]
    ...
    def print_final_summary(self) -> bool:
        ...
        if passed == total:
            ...
            logger.warning(f"⚠️  {total - passed} test suite(s) failed. Please review the logs above.")
            return False
