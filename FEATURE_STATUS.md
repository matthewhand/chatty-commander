...
| Repo hygiene | ❌ | A tracked junk file literally named `javascript:alert('xss')` sits at the repo root (`git ls-files` confirms it is tracked); untracked clutter also present (`MagicMock/`, `htmlcov/`, root `node_modules/`, `test_results_*.txt`, and a stale `failing_tests.txt` predating the current green suite); legacy unused `frontend/web-app/` and `frontend/desktop-app/` trees still present (current app is `webui/frontend/`) |
